...
extern void sys_dump_u8(uint32_t address, uint32_t len);
...
int cli_qspi(int argc, char **argv)
{
    ...
    else if ((strcmp(argv[0], "-r") == 0) || (strcmp(argv[0], "--read") == 0))
    {
        ...
        sys_dump_u8((uint32_t) pdata, size);
    ...
    else if (strcmp(argv[0], "--erase_sector") == 0)
    {
        ...
        for (uint32_t i = 0; i < sector_num; i++)
        {
            ...
            printf("\nErasing Sector [%4ld], Erased = [%8ld kB]", sector_start + i, (i + 1) * (info.SectorSize));
        }
